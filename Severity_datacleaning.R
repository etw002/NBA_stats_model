...
stats_ds <- boxscore_raw_data %>% 
  ...
  dplyr::select(game_id,athlete_display_name,points,fg,ft,starter,athlete_position_abbreviation)
...
usage_data <- boxscore_raw_data %>%
  ...
  dplyr::select(game_id,athlete_display_name,usage_rate,player_poss,team_poss,minutes,
         team_minutes)

ppg_modeling_data <- stats_ds %>%
  left_join(usage_data, by = c("game_id","athlete_display_name")) %>%
  dplyr::select(game_id,athlete_display_name, points, usage_rate, fg, ft, starter, 
         athlete_position_abbreviation)

saveRDS(ppg_modeling_data, "Severity_ds.rds")