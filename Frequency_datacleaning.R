...
year_played_data <- allplayers_raw_data %>%
  ...
  dplyr::select(PERSON_ID,DISPLAY_FIRST_LAST,years_played)
...
dnp_why <-boxscore_raw_data %>%
  ...
  dplyr::select(athlete_id,athlete_display_name,did_not_play,reason) %>%
...
modeling_data <- player_totals_clean %>%
  ...
  left_join(
    dnp_why_clean %>% 
      dplyr::select(athlete_id, games_missed_injury), 
    by = "athlete_id"
  ) %>%
...
saveRDS(modeling_data, "Frequency_ds.rds")